...
type installer struct {
	...
	ipxeBaseURL string
	// osieURL is the default base URL for the OSIE installer
	osieURL string
...
func Installer(dataModelVersion, tinkGRPCAuth, extraKernelArgs, registry, registryUsername, registryPassword, ipxeBaseURL string, tinkTLS bool) job.BootScripter {
	defaultParams := []string{
		"ip=dhcp",
...
func (i installer) bootScript(ctx context.Context, action string, j job.Job, s *ipxe.Script) {
	...
	s.Set("base-url", i.osieBaseURL(j))
...
func (i installer) osieBaseURL(j job.Job) string {
	if u := j.OSIEBaseURL(); u != "" {
		return u
	}
	if isCustomOSIE(j) {
		return i.osieURL + "/" + j.OSIEVersion()
	}

	if i.ipxeBaseURL != "" {
		return strings.TrimRight(i.ipxeBaseURL, "/")
	}

	return i.osieURL + "/current"
}
