...
# Fetch packet MD
wget https://metadata.packet.net/metadata -O /tmp/metadata
...
if [ "$sshset" == "true" ]; then
	...
	wget -q https://metadata.packet.net/2009-04-04/meta-data/public-keys -O /etc/ssh/keys-root/authorized_keys
...
wget https://metadata.packet.net/2009-04-04/meta-data/public-keys -O /etc/ssh/keys-root/authorized_keys
...
#/bin/sh
metadata=/tmp/metadata
wget https://metadata.packet.net/metadata -O $metadata
